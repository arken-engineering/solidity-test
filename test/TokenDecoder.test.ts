...
describe("TokenDecoder", () => {
    ...
    describe("Decode Runeword", async () => {
        ...
        const attributeCountToLowestGasMap = [0, 27034, 0, 30356, 32626, 34044, 36046, 38048, 39904]
        const testDecode = async (expected: Runeword): Promise<void> => {
            ...
            if (logGas) {
                console.log(
                    "\t- %d gas, lowest to date %d",
                    (await tokenDecoderContract.estimateGas.decodeRuneword(expected.token)).toNumber(),
                    attributeCountToLowestGasMap[expected.attributeCount],
                )
            }
        }
